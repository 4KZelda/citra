package org.citraemu.citraemu.model;
				...
				for (File file : children)
				{
					if (!file.isHidden() && !file.isDirectory())
					{
						...
						if (extensionStart > 0)
						{
							...
							{
								String name = NativeLibrary.GetShortTitle(filePath);
								...
								String gameId = NativeLibrary.GetProgramId(filePath);
								...
								int platform = Game.PLATFORM_GC;

								ContentValues game = Game.asContentValues(platform,
										name,
										NativeLibrary.GetLongTitle(filePath),
										Game.COUNTRY_UNKNOWN, //FIXME read from smdh.region_lockout
										filePath,
										gameId,
										NativeLibrary.GetPublisher(filePath));
